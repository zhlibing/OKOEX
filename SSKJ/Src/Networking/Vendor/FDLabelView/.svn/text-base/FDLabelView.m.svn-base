...
-(void)drawDebug:(NSArray*)linebreaks{
    ...
    if(showLog){
        SSLog(@"==== FDLabelView =============");
        SSLog(@"origin:             (%.1f, %.1f)", self.frame.origin.x, self.frame.origin.y);
        SSLog(@"center:             (%.1f, %.1f)", self.frame.origin.x + center.x, self.frame.origin.y + center.y);
        SSLog(@"size:               (%.1f, %.1f)", self.frame.size.width, self.frame.size.height);
        SSLog(@"font-size:           %.1f /%.1f", displayFont.pointSize, self.font.pointSize);
        SSLog(@"==============================");
    }
}
    ...
    [textLine enumerateSubstringsInRange:NSMakeRange(0, textLine.length)
                              ...
                              usingBlock:^(NSString *word,
                                  ...
                                  if([enclosingWord canBeConvertedToEncoding:NSASCIIStringEncoding]){
                                      ...
                                      SSLog(@"add word [%@]", [textLine substringWithRange:range]);
                                  }else{
