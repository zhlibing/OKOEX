...
{
    NSData *data = [RSA encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] privateKeyPath:privKeyPath];
    NSString *ret = base64_encode_data(data);
    return ret;
}
...
+ (NSData *)encryptData:(NSData *)data privateKeyPath:(NSString *)privKeyPath{
    if(!data || !privKeyPath){
        return nil;
    }
    SecKeyRef keyRef = [RSA getPrivateKeyRefWithContentsOfFile:privKeyPath password:PrivateKeyPassword];
    if(!keyRef){
        return nil;
    }
    return [RSA encryptData:data withKeyRef:keyRef isSign:YES];
...
{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [RSA decryptData:data privateKeyPath:privKeyPath];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
...
+ (NSData *)decryptData:(NSData *)data privateKeyPath:(NSString *)privKeyPath{
    if(!data || !privKeyPath){
        return nil;
    }
    SecKeyRef keyRef = [RSA getPrivateKeyRefWithContentsOfFile:privKeyPath password:PrivateKeyPassword];
...
{
    NSData *data = [RSA encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicKeyPath:pubKeyPath];
    NSString *ret = base64_encode_data(data);
    return ret;
}
...
+ (NSData *)encryptData:(NSData *)data publicKeyPath:(NSString *)pubKeyPath{
    if(!data || !pubKeyPath){
        return nil;
    }
    SecKeyRef keyRef = [RSA getPublicKeyRefWithContentsOfFile:pubKeyPath];
    if(!keyRef){
        return nil;
    }
    return [RSA encryptData:data withKeyRef:keyRef isSign:NO];
...
{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [RSA decryptData:data publicKeyPath:pubKeyPath];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}


+(NSData *)decryptData:(NSData *)data publicKeyPath:(NSString *)pubKeyPath
{
    SecKeyRef keyRef = [RSA getPublicKeyRefWithContentsOfFile:pubKeyPath];
    if(!keyRef){
        return nil;
