...
#import "Mine_PayWayAdd_ShowView.h"
#import "Mine_PayWayAdd_Cell.h"
#import "Mine_PayWay_WaySelectView.h"

#import "FRCameraViewController.h"
#import "Mine_PayWay_SelectPhotoView.h"
#import "ManagerSocket.h"
#import "SSKJ_Default_ActionsheetView.h"

@interface Mine_PayWayAdd_ShowView()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@property (nonatomic, strong) Mine_PayWay_WaySelectView *alertView;
@property (nonatomic, strong) Mine_PayWay_SelectPhotoView *selectPhoto;
...
@implementation Mine_PayWayAdd_ShowView
...
-(Mine_PayWay_WaySelectView *)alertView{
    if (!_alertView) {
        _alertView = [[Mine_PayWay_WaySelectView alloc]initWithFrame:CGRectMake(_btn.x, _btn.maxY, 90 , 240)];
...
{
    Mine_PayWayAdd_Cell *cell = [tableView dequeueReusableCellWithIdentifier:[NSString stringWithFormat:@"Mine_PayWayAdd_Cell%ld%ld",indexPath.row,self.selectType]];
    WS(weakSelf);
    if (cell == nil) {
        cell = [[Mine_PayWayAdd_Cell alloc]initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:[NSString stringWithFormat:@"Mine_PayWayAdd_Cell%ld%ld",indexPath.row,self.selectType]];
...
-(Mine_PayWay_SelectPhotoView *)selectPhoto
{
    if (!_selectPhoto) {
        _selectPhoto = [[Mine_PayWay_SelectPhotoView alloc]init];
        UIImagePickerController *imagePickerController = [[UIImagePickerController alloc] init];
        imagePickerController.delegate = self;
        imagePickerController.allowsEditing = YES;
        WS(weakSelf);
